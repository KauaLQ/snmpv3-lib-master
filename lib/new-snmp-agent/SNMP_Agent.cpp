...
void SNMPAgent::setUDP(UDP* udp){
    ...
    // Movido para o final para garantir que begin() seja chamado após a configuração
    // this->begin();
}

void SNMPAgent::begin(){
    // <<< 1. INICIALIZAR O MÓDULO USM
    this->usm.begin();
    this->restartUDP();
}
...
SNMP_ERROR_RESPONSE SNMPAgent::loop(){
    for(auto udp : _udp){
        ...
        if(packetLength > 0){
            ...
            // <<< CORREÇÃO PRINCIPAL: Passar os novos parâmetros v3 para handlePacket
            SNMP_ERROR_RESPONSE response = handlePacket(_packetBuffer, packetLength, &responseLength, MAX_SNMP_PACKET_LENGTH, callbacks, _community, _readOnlyCommunity, usm, _v3_users, _num_v3_users, informCallback, (void*)this);
...
// <<< 3. IMPLEMENTAÇÃO DA NOVA FUNÇÃO addUser
bool SNMPAgent::addUser(const char* userName, SNMPV3SecurityLevel level, SNMPV3AuthProtocol authProto, const char* authPass, SNMPV3PrivProtocol privProto, const char* privPass) {
    if (_num_v3_users >= MAX_V3_USERS) {
        SNMP_LOGW("Não é possível adicionar mais usuários v3. Limite atingido.");
        return false;
    }

    // Pega o próximo slot de usuário disponível
    SNMPV3User* newUser = &_v3_users[_num_v3_users];

    // Copia os detalhes do usuário para a estrutura
    strncpy(newUser->userName, userName, 32);
    newUser->userName[32] = '\0'; // Garante terminação nula

    newUser->securityLevel = level;

    if (level >= AUTH_NO_PRIV) {
        strncpy(newUser->authPassword, authPass, 63);
        newUser->authPassword[63] = '\0';
        newUser->authProtocol = authProto;
    }

    if (level == AUTH_PRIV) {
        strncpy(newUser->privPassword, privPass, 63);
        newUser->privPassword[63] = '\0';
        newUser->privProtocol = privProto;
    }

    // Passo CRÍTICO: Gera as chaves criptográficas a partir das senhas
    if (!usm.passwordToKey(*newUser)) {
        SNMP_LOGW("Falha ao gerar chaves para o usuário: %s", userName);
        return false;
    }

    SNMP_LOGD("Usuário v3 '%s' adicionado com sucesso.", userName);
    _num_v3_users++;
    return true;
}
